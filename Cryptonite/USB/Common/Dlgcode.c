...
BOOL DoDriverInstall (HWND hwndDlg)
{
	...
	hService = CreateService (hManager, "cryptonite", "cryptonite",
		SERVICE_ALL_ACCESS, SERVICE_KERNEL_DRIVER, SERVICE_SYSTEM_START, SERVICE_ERROR_NORMAL,
		"System32\\drivers\\cryptonite.sys",
	...
	hService = OpenService (hManager, "cryptonite", SERVICE_ALL_ACCESS);
	if (hService == NULL)
		goto error;
...
static int DriverLoad ()
{
	...
	if (ReadLocalMachineRegistryDword ("SYSTEM\\CurrentControlSet\\Services\\cryptonite", "Start", &startType) && startType == SERVICE_BOOT_START)
	...
	strcpy (tmp, !Is64BitOs () ? "\\cryptonite.sys" : "\\cryptonite-x64.sys");
