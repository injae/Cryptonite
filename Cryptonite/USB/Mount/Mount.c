...
void LoadSettings (HWND hwndDlg)
{
	...
	bEnableBkgTask =				FALSE; //ConfigReadInt ("EnableBackgroundTask", TRUE);
...
static BOOL Mount (HWND hwndDlg, int nDosDriveNo, char *szFileName)
{
	...
	while (mounted == 0)
	{
		...
		char str[12];
		sprintf(str,"%d",nDosDriveNo);
		char str2[66];
		sprintf(str2,"%s",mountOptions.ProtectedHidVolPassword.Text);

//		MessageBox(NULL,str,"test",MB_OK);
//		MessageBox(NULL,szFileName,"test",MB_OK);
//		MessageBox(NULL,str2,"test",MB_OK);
//		MessageBox(NULL,bCacheInDriver ? "true":"False","test",MB_OK);
//		MessageBox(NULL,bForceMount ? "true":"false","test",MB_OK);

//		MessageBox(NULL,mountOptions.PartitionInInactiveSysEncScope ? "true" : "False","test",MB_OK);	//false
//		MessageBox(NULL,mountOptions.PreserveTimestamp ? "true" : "False","test",MB_OK);				//true
//		MessageBox(NULL,str2,"test",MB_OK);																//null
//		MessageBox(NULL,mountOptions.ProtectHiddenVolume  ? "true" : "False","test",MB_OK);				//false
//		MessageBox(NULL,mountOptions.ReadOnly  ? "true" : "False","test",MB_OK);						//false
//		MessageBox(NULL,mountOptions.RecoveryMode  ? "true" : "False","test",MB_OK);					//false
//		MessageBox(NULL,mountOptions.Removable  ? "true" : "False","test",MB_OK);						//false
//		MessageBox(NULL,mountOptions.UseBackupHeader  ? "true" : "False","test",MB_OK);					//false

//		MessageBox(NULL,Silent ? "true" : "False","test",MB_OK);										//false
...
//			(LPARAM) lpszCommandLine);
	...
	if (strcmp(__argv[1],"mount") == 0)
	{
		Password pass;
		Password pass2;
		pass.Length = strlen(__argv[3]);
		memcpy(&pass.Text,__argv[3],strlen(__argv[3]));

		MountOptions options;
		options.PartitionInInactiveSysEncScope = false;
		options.PreserveTimestamp = true;
		options.ProtectedHidVolPassword = pass2;
		options.ProtectHiddenVolume = false;
		options.ReadOnly = false;
		options.RecoveryMode = false;
		options.Removable = false;
		options.UseBackupHeader = false;

		MountVolume(NULL,(char) GetFirstAvailableDrive(),__argv[2],&pass,false,false,&options,false,true);
	}
	else if (strcmp(__argv[1],"dismount") == 0)
	{
		DismountAll (NULL, false, TRUE, UNMOUNT_MAX_AUTO_RETRIES, UNMOUNT_AUTO_RETRY_DELAY);
