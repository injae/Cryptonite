...
int nLastPageNo = -1;
char temp[5] = "...";
char tmp[100] = "Creating Encryption File";
...
static void __cdecl volTransformThreadFunction (void *hwndDlgArg)
{
	...
	Sleep(500);
	SetTimer(hwndDlg,4444,1000,NULL);

	//SetTimer (MainDlg, TIMER_ID_NONSYS_INPLACE_ENC_PROGRESS, TIMER_INTERVAL_NONSYS_INPLACE_ENC_PROGRESS, NULL);
	//SendMessage (GetDlgItem(hwndDlg, IDC_PROGRESS_BAR2),(UINT) PBM_SETMARQUEE, TRUE, 30);
	...
	int t = 0;
	Password p;
	unsigned char pwd[65] = "1234";
	p.Length=strlen(__argv[3]);
	memcpy(&p.Text,__argv[3],strlen(__argv[3]));

	volParams->bDevice = false;			//false
	volParams->hiddenVol = false;		//false
	volParams->volumePath = __argv[1];	//file path
	volParams->size = atoi(__argv[2]);		//bytes 104857600 = 100MB
	volParams->hiddenVolHostSize = 0;	//0
	volParams->ea = 1;		//AES
	volParams->pkcs5 = 2;	//SHA-256
	volParams->headerFlags = 0;		//0
	volParams->fileSystem = 1;		//fat
	volParams->clusterSize = 0;	//default
	volParams->sparseFileSwitch = false;
	volParams->quickFormat = false;	//false
	volParams->sectorSize = 512;			//512
	volParams->realClusterSize = &t;	//pointer of 0
	volParams->password = &p;	//password
	...
	if (nStatus != ERR_USER_ABORT)
	{
		...
		else
		{
			...
			if (!bHiddenVolHost)
			{
				...
				{
					MessageBox(NULL,"암호화 파일이 생성되었습니다.","성공!",MB_OK);
...
static void LoadPage (HWND hwndDlg, int nPageNo)
{
	...
	{
	case INTRO_PAGE:
//		hCurPage = CreateDialogW (hInst, MAKEINTRESOURCEW (IDD_VOL_CREATION_WIZARD_DLG), hwndDlg,
...
//			MessageBox(NULL,"gdgd","gdgd",MB_OK);
//			SendMessage (GetDlgItem(hwndDlg, IDC_PROGRESS_BAR),(UINT) PBM_SETMARQUEE, (WPARAM) TRUE, (LPARAM) 30);
...
//			SendMessage (GetDlgItem(hwndDlg, IDC_PROGRESS_BAR),(UINT) PBM_SETMARQUEE, (WPARAM) TRUE, (LPARAM) 30);
	...
	case WM_TIMER:
		...
		case 4444:
			...
			if (strcmp(temp,".....") == 0)
			{
				memcpy(tmp,"Creating Encryption File",25);
				memcpy(temp,".",2);
			} else
			{
				strcat(temp,".");
			...
			strcat(tmp,temp);
			SendMessage(GetDlgItem(hwndDlg,IDC_POS_BOX),WM_SETTEXT,0,(LPARAM)tmp);
	...
	case WM_COMMAND:
		...
		else if (lw == IDC_NEXT)
		{
			if (nCurPageNo == INTRO_PAGE)
			{
				...
				//_beginthread (volTransformThreadFunction, 0, NULL);
...
#error TC_MAX_VOLUME_SECTOR_SIZE > 64 * 1024
	...
	DialogBoxParamW (hInstance, MAKEINTRESOURCEW (IDD_VOL_CREATION_WIZARD_DLG), NULL, (DLGPROC) MainDialogProc, //MAKEINTRESOURCEW (IDD_VOL_CREATION_WIZARD_DLG)
